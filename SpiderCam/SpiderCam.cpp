...
void setNextInterruptInterval() {
  ...
  for (int i = 0; i < NUM_STEPPERS; i++) {
    if ( ((1 << i) & remainingSteppersFlag) && steppers[i].scaledDelay < mind ) {
...
ISR(TIMER1_COMPA_vect)
{
  ...
  for (int i = 0; i < NUM_STEPPERS; i++) {
    ...
    if ( s.rampUpStepCount == 0 ) {
      s.accIndex++;
      s.currentDelay = s.currentDelay - (2 * s.currentDelay) / (4 * s.accIndex + 1);
      ...
      s.currentDelay = (s.currentDelay * (4 * s.accIndex + 1)) / (4 * s.accIndex + 1 - 2);
      s.accIndex--;
    }
